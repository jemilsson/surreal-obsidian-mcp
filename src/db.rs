...
pub struct Block {
    ...
    // Metadata
    pub properties: HashMap<String, serde_json::Value>, // Frontmatter for level 0
    pub tags: Vec<String>,
...
impl Database {
    ...
    async fn init_schema(&self) -> Result<()> {
        ...
        self.db
            .query(
                "
                DEFINE TABLE IF NOT EXISTS blocks SCHEMAFULL;
                DEFINE FIELD IF NOT EXISTS id ON blocks TYPE string;
                DEFINE FIELD IF NOT EXISTS level ON blocks TYPE int;
                DEFINE FIELD IF NOT EXISTS title ON blocks TYPE string;
                DEFINE FIELD IF NOT EXISTS content ON blocks TYPE string;
                DEFINE FIELD IF NOT EXISTS file_path ON blocks TYPE string;
                DEFINE FIELD IF NOT EXISTS parent_id ON blocks TYPE option<string>;
                DEFINE FIELD IF NOT EXISTS children_ids ON blocks TYPE array<string>;
                DEFINE FIELD IF NOT EXISTS properties ON blocks TYPE object;
                DEFINE FIELD IF NOT EXISTS tags ON blocks TYPE array<string>;
                DEFINE FIELD IF NOT EXISTS created_at ON blocks TYPE int;
                DEFINE FIELD IF NOT EXISTS updated_at ON blocks TYPE int;
                DEFINE FIELD IF NOT EXISTS embedding ON blocks TYPE option<array<float>>;
                DEFINE FIELD IF NOT EXISTS outgoing_links ON blocks TYPE array<string>;
                DEFINE FIELD IF NOT EXISTS incoming_links ON blocks TYPE array<string>;
